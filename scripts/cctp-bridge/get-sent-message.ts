...
const sendTx =
  '0xe7a7f6b02fec98f6c13142f0e848a79a5a0b3714dbc7815750e81a7c986e5dd6';

async function main() {
  const messageBytes = await getSentMessage(sendTx);
  console.log('sentMessage:', messageBytes);
}

async function getSentMessage(txId: string): Promise<string> {
  const txReceipt = await ethers.provider.getTransactionReceipt(txId);
  if (!txReceipt) {
    throw Error(`Transaction ${txId} not found.`);
  }
  const messageSentTopic = ethers.utils.id('MessageSent(bytes)');
  const log = txReceipt.logs.find((l) => l.topics[0] === messageSentTopic);
  if (!log) {
    throw Error(`Event 'MessageSent' not found`);
  }
